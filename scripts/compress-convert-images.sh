...
echo "Checking for files not in WebP format in src/img/ folder..."

# Find files that are not in WebP format and are not .DS_STORE files
non_webp_files=$(find src/img/ -type f ! -name "*.webp" -not -name ".DS_STORE")
...
if [[ -n "$non_webp_files" ]]; then
    ...
    find src/img -type f \( -name "*.png.webp" -o -name "*.png" -o -name "*.jpg.webp" -o -name "*.jpg" -o -name "*.jpeg.webp" -o -name "*.jpeg" \) | while IFS= read -r file; do
        if [[ $file == *.png* ]]; then
